  ...
  saveResponse(status, tmpl) {
    ...
    if (status === 'Submit') {
      ...
      switch (getSurvey.stype) {
        case 'spdat-t':
          score = spdatScoreHelpers.calcSpdatTayScore(tmpl.data.survey._id, client._id);
          break;
        case 'spdat-f':
          score = spdatScoreHelpers.calcSpdatFamilyScore(tmpl.data.survey._id, client._id);
          break;
        case 'spdat-s':
          score = spdatScoreHelpers.calcSpdatSingleScore(tmpl.data.survey._id, client._id);
          break;
        default:
          score = 0;
          break;
      ...
      if (score !== 0) {
        Meteor.call(
          'sendScoresToHMIS', tmpl.data.survey._id, client._id, score, (err, result) => {
            if (err) {
              logger.log(err);
            } else {
              logger.log(result);
