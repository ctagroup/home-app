...
responseHmisHelpers = {
  sendResponseToHmis(_id, responseToSend = {}, fromDb = false, callback) {
    ...
    if (fromDb) {
      ...
      logger.info(`response Mongo Id: ${JSON.stringify(response, null, 2)}`);
      // Getting Client Id.
      const clientId = response.clientID;
      // Getting App Id.
      const config = ServiceConfiguration.configurations.findOne({ service: 'HMIS' });
      if (!config) {
        callback('Service configuration not found.');
      }
      ...
      if (surveyDetails.apiSurveyServiceId) {
        ...
        responseHmisHelpers.sendResponses(clientId, surveyId, appId,
          response.section, (err, result) => {
        ...
        callback(`Survey Id does not exist: ${response.surveyID}`);
      }
    } else {
      ...
      if (surveyDetails.apiSurveyServiceId) {
        ...
        const surveyId = surveyDetails.apiSurveyServiceId;
        responseHmisHelpers.sendResponses(clientId, surveyId, appId,
          responseToSend.section, (err, result) => {
            // Add id to local response.
            callback(err, result);
          });
      } else {
        ...
        callback(`Survey Id does not exist: ${responseToSend.surveyID}`);
      }
    }
  },
  sendResponses(clientId, surveyId, appId, sections, callback) {
    ...
    for (let i = 0; i < sections.length; i += 1) {
      ...
      if (!sectionDetails.apiSurveyServiceId) {
        ...
        callback(`Section '${sectionDetails.content}' not uploaded!`);
      } else {
