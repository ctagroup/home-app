...
import { Clients } from '/imports/api/client-side-collections';

import { WaitOn } from '/imports/utils/wait-on';
        ...
        edit: {
          ...
          waitOn() {
            ...
            if (this.params.query && this.params.query.schema) {
              return WaitOn.method('getClient', id, (error, client) => {
                if (error) {
                  // TODO: handle error
                  Bert.alert(error.reason || error.error, 'danger', 'growl-top-right');
                } else {
                  Clients.upsert(client.clientId, client);
                }
              });
            }
