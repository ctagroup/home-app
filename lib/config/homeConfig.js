        ...
        new: {
          ...
          waitOn() {
            if (this.params.query && this.params.query.schema) {
              const _id = this.params.query.clientId;
              return Meteor.subscribe('clients.one', _id, this.params.query.schema);
            }
          ...
          data() {
            const client = PendingClients.findOne({ _id: this.params.query.clientId }) ||
              Clients.findOne(this.params.query.clientId);
            if (client && this.params.query && this.params.query.schema) {
              client.isHMISClient = true;
            }
            const survey = Surveys.findOne({ _id: this.params.query.surveyId });
