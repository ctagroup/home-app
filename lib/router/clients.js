  ...
  '/clients/:_id', {
    ...
    waitOn() {
      if (this.params.query && this.params.query.schema) {
        return [
          Meteor.subscribe('client', Router.current().params._id),
          // Meteor.subscribe('eligibleClient', Router.current().params._id),
        ];
      }
      return Meteor.subscribe('pendingClient', Router.current().params._id);
    },
    ...
    data() {
      return PendingClients.findOne({ _id: this.params._id }) ||
        Clients.findOne({ _id: this.params._id });
    },
  }
);
