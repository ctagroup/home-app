...
spdatScoreHelpers = {
  ...
  calcSpdatTayScore(surveyId, clientId) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const temp = { sectionId, surveyId, clientId, sectionScore };
    ...
    for (let i = 0; i < surveySections.length; i++) {
      ...
      if (sectionScores[0].sectionScore === 0) {
        if (/basic/i.test(surveySections[i].content)) {
          ...
          sectionScores[0].sectionId = surveySections._id;    // save details of sectionId
      ...
      if (sectionScores[1].sectionScore === 0) {
        if (/homelessness/i.test(surveySections[i].content)) {
          sectionScores[1].sectionId = surveySections._id;    // save details of sectionId
          const qIds = _.pluck(surveyQuestionsMaster.find(
            { $and: [
              { sectionID: surveySections[i]._id },
              { surveyID: surveyId },
      ...
      if (sectionScores[2].sectionScore === 0) {
        if (/risk/i.test(surveySections[i].content)) {
          sectionScores[2].sectionId = surveySections._id;    // save details of sectionId
          const qIds = _.pluck(surveyQuestionsMaster.find(
            { $and: [
              { sectionID: surveySections[i]._id },
              { surveyID: surveyId },
      ...
      if (sectionScores[3].sectionScore === 0) {
        if (/socialization/i.test(surveySections[i].content)) {
          sectionScores[3].sectionId = surveySections._id;    // save details of sectionId
          const qIds = _.pluck(surveyQuestionsMaster.find(
            { $and: [
              { sectionID: surveySections[i]._id },
              { surveyID: surveyId },
      ...
      if (sectionScores[4].sectionScore === 0) {
        if (/wellness/i.test(surveySections[i].content)) {
          sectionScores[4].sectionId = surveySections._id;    // save details of sectionId
          const qIds = _.pluck(surveyQuestionsMaster.find(
            { $and: [
              { sectionID: surveySections[i]._id },
              { surveyID: surveyId },
    ...
    logger.log(`${sectionScores[0].sectionScore} ${sectionScores[1].sectionScore}`);
    logger.log(`${sectionScores[2].sectionScore} ${sectionScores[3].sectionScore}`);
    logger.log(`${sectionScores[4].sectionScore}`);
    return sectionScores;
  },
  calcSpdatSingleScore(surveyId, clientId) {
    // Get all sections of this survey.
    const surveySections = surveyQuestionsMaster.find(
      { $and: [
        { surveyID: surveyId },
  ...
  calcSpdatFamilyScore(surveyId, clientId) {
    // Get all sections of this survey.
    ...
    let surveySections = surveyQuestionsMaster.find(
      { $and: [
        { surveyID: surveyId },
        { contentType: { $eq: 'section' } },
      ] }
    ).fetch();
    surveySections = surveyFormatHelpers.getSections(surveyId);
    // Initializing the sectionScores as defined in API - Survey Service 1.0.0
    const sectionScores = [];
    for (let i = 0; i < 6; i++) {
      const sectionId = '';
    ...
    for (let i = 0; i < surveySections.length; i++) {
      ...
      if (/parent 2/i.test(surveySections[i].content)) {
        ...
        // Get their answers from UI and score them according to response
        const ansAge2 = spdatScoreHelpers.getAnswer(
          Age2._id, Age2.dataType, surveySections[i]._id
        );
        // Save the value to section Scores.
      ...
      if (/children/i.test(surveySections[i].content)) {
        ...
        const ansQ4 = spdatScoreHelpers.getAnswer(Q4._id, Q4.dataType, surveySections[i]._id);
      ...
      if (sectionScores[5].sectionScore === 0) {
        if (/family unit/i.test(surveySections[i].content)) {
          sectionScores[5].sectionId = surveySections._id;    // save details of sectionId
          const qIds = _.pluck(surveyQuestionsMaster.find(
            { $and: [
              { sectionID: surveySections[i]._id },
              { surveyID: surveyId },
    ...
    logger.log(`${sectionScores[0].sectionScore} ${sectionScores[1].sectionScore}`);
    logger.log(`${sectionScores[2].sectionScore} ${sectionScores[3].sectionScore}`);
    logger.log(`${sectionScores[4].sectionScore} ${sectionScores[5].sectionScore}`);
    return sectionScores;
  },
};
