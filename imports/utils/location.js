...
function makeGeocodeAPICall(location) {
  ...
  // multiple "alt" addresses, user should confirm which one because the main address isn't
...
export function getAddressFromLatLong(latLng) {
  ...
  const stnumber = response.stnumber ? response.stnumber : '';
  const staddress = response.staddress ? response.staddress : '';
  ...
  const postal = response.postal ? response.postal : '';
...
export function getLatLongFromAddressOrDevice(address) {
  ...
  if (address !== null) {
    // TODO: Improve this to add USA if not already there, if country isn't
    // specified the result can be incorrect
