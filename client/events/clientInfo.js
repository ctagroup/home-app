...
Template.createClient.events({
  'click .save'(evt, tmpl) {
    ...
    Meteor.call('addClient', firstName, middleName, lastName, suffix, ssn,
...
Template.viewClient.events({
  ...
  'click .add-to-hmis'(tmpl) {
    Meteor.call('addClientToHMIS', tmpl.data._id, (error, result) => {
      if (error) {
        // console.log(error);
      } else {
        const query = (result) ? 'addedToHMIS=1' : 'addClientToHMISError=1';
...
Template.editClient.events({
  'click .update'(evt, tmpl) {
    ...
    const loc = tmpl.find('.destination_category').value;
    const shelter = tmpl.find('.timeOnStreets_category').value;
    Meteor.call('updateClient', tmpl.data._id, firstName, middleName, lastName,
  ...
  'click .delete'(evt, tmpl) {
    Meteor.call('removeClient', tmpl.data._id, (error) => {
      if (error) {
        // console.log(error);
      } else {
        // console.log(result);
